...
func (vsc *virtualServerConfigurator) GenerateVirtualServerConfig(virtualServerEx *VirtualServerEx, tlsPemFileName string) (version2.VirtualServerConfig, Warnings) {
	...
	for _, r := range virtualServerEx.VirtualServer.Spec.Routes {
		...
		if len(r.Matches) > 0 {
			...
			loc := generateLocation(r.Path, upstreamName, upstream, r.Action, vsc.cfgParams, r.Snippet)
...
func generateLocation(path string, upstreamName string, upstream conf_v1.Upstream, action *conf_v1.Action, cfgParams *ConfigParams, snippet string) version2.Location {
	...
	return generateLocationForProxying(path, upstreamName, upstream, cfgParams, snippet)
}

func generateLocationForProxying(path string, upstreamName string, upstream conf_v1.Upstream, cfgParams *ConfigParams, snippet string) version2.Location {
...
func generateSplits(splits []conf_v1.Split, upstreamNamer *upstreamNamer, crUpstreams map[string]conf_v1.Upstream, variableNamer *variableNamer, scIndex int, cfgParams *ConfigParams) (version2.SplitClient, []version2.Location) {
	...
	for i, s := range splits {
		...
		upstream := crUpstreams[upstreamName]
		loc := generateLocation(path, upstreamName, upstream, s.Action, cfgParams, "") //TODO
		locations = append(locations, loc)
	}
...
func generateMatchesConfig(route conf_v1.Route, upstreamNamer *upstreamNamer, crUpstreams map[string]conf_v1.Upstream,
	...
	if len(route.Splits) > 0 {
		...
		upstream := crUpstreams[upstreamName]
		loc := generateLocation(path, upstreamName, upstream, route.Action, cfgParams, route.Snippet)
		locations = append(locations, loc)
	}
